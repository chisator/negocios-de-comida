  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center p-6 text-center">
      <div className={`w-20 h-20 rounded-full flex items-center justify-center mb-6 ${
        ...
        <CheckCircle className={`w-10 h-10 ${isApproved ? "text-emerald-400" : "text-amber-400"}`} />
      </div>

      <h1 className="text-2xl font-bold text-white mb-2">
        {isApproved ? "¡Pago confirmado!" : "¡Pedido confirmado!"}
      </h1>
      <p className="text-zinc-400 text-sm mb-8 max-w-xs">
        {method === "mercadopago"
          ? isApproved
            ? "Tu pago fue procesado. Te avisaremos cuando tu pedido esté listo."
            : "Estamos esperando la confirmación del pago. Te avisaremos por WhatsApp."
